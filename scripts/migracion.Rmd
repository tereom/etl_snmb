...
Archivo_especie_invasora <- archivos_invasora %>%
  ...
  select(id, especie_invasora_id = invaders_id, archivo_nombre_original, 
         archivo)

Especie_invasora <- invasora %>%
...
Especie_invasora <- transecto_especies_invasoras %>%
  mutate(
    ...
    numero_individuos = as.character(cut(number_individuals, 
      breaks = c(-Inf, 0, 5, 10, 90, 99), 
      labels = c("No aplica", "1 a 5", "6 a 10", "más de 10", "No aplica"))),
