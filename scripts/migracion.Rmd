...
transecto_huellas_excretas <- huella_excreta %>%
  ...
  group_by(conglomerate_id, transecto_numero) %>%
  mutate(
    transecto_huella_excreta_id = first(id)
  ) %>%
  ungroup()


Transecto_huellas_excretas_muestra <- transecto_transecto_huellas_excretas %>%
  group_by(conglomerate_id, transecto_numero) %>%
  summarise_each(funs(first)) %>%
  ungroup() %>%
  mutate(
  ...
  select(id = transecto_huella_excreta_id, 
    conglomerado_muestra_id = conglomerate_id, fecha,
    transecto_numero, tecnico, hora_inicio, hora_termino, comentario)


Especie_invasora <- transecto_transecto_huellas_excretas %>%
