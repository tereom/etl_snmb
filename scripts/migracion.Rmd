...
rm("cat_municipio", "conglomerado", "conglomerado_tabs", "sitio")
```
...
camara <- camara_tabs %>%
  ...
  filter((conglomerate_id %in% Conglomerado_muestra$id))  %>%
    ...
    # por lo que favorecemos la tabla equip_camera
...
# conglomerado-sitio, después unimos con sitios_cgls para obetebr sitio_muesrea_id
camara_aux <- camara %>%
  group_by(location) %>%
  summarise_each(funs(first))
...
Camara <- camara_sitio %>%
  mutate(
    ...
    resolucion = resolution,  ### ingresada sin menú regex?
  ...
  select(id = id.x, #nos quedamos con id de la tabla trap_camera
    ...
    resolucion, sensibilidad,
    comentario
    )

camara_sitio$resolucion[grep("1920", camara_sitio$resolucion)] <- "2MP"
camara_sitio$resolucion[grep("4800", camara_sitio$resolucion)] <- "5MP"
camara_sitio$resolucion[grep("11520", camara_sitio$resolucion)] <- "12MP"


# seleccionamos los campos de la tabla trap_camera que formarán parte de
# la tabla Archivo_camara
trap_camera_sub <- select(camara, trap_camera_id = id, number_individuals, 
  species_name, type_first_image, type_image, type_video)

archivos_camara_2 <-  left_join(archivos_camara, trap_camera_sub, 
  by ="trap_camera_id")

Archivo_camara <- archivos_camara_2 %>%
  mutate(
    presencia = (number_individuals > 0),
    numero_individuos = ifelse(presencia, number_individuals, NA)
  ) %>%
  select(id, 
    camara_id = trap_camera_id, 
    archivo = filename, # regex
    archivo_nombre_original = old_filename, # regex
    presencia, 
    nombre_comun = species_name,  # como decidir?
    nombre_cientifico = species_name, 
    numero_individuos
    )

# unimos con camera_sitio y equip_camera
camara_equipo <- camara_sitio %>%
  select(
    equipment_id = id.y, 
    camara_id = id.x
  )
...
soundscape <- soundscape_tabs %>%
  ...
  filter((conglomerate_id %in% Conglomerado_muestra$id))  %>%
    # Puede haber casos donde equip_microphone tiene vacíos y soundscape no
    # por lo que favorecemos la tabla soundscape
  select(-.id)
...
archivos_microfono <- archivos_microfono_tabs %>% 
  ...
  filter(soundscape_id %in% soundscape$id) %>%
  select(-.id)
...
# la tabla Archivo_grabadora
soundscape_sub <- select(soundscape, soundscape_id = id, type_soundscape, 
  type_ultrasonic)

archivos_grabadora_2 <-  left_join(archivos_microfono, soundscape_sub, 
  by = "soundscape_id")

Archivo_grabadora <- archivos_grabadora_2 %>%
  mutate(
    es_audible = type_soundscape == 1
  ) %>%
  select(id, 
    grabadora_id = soundscape_id, 
    archivo = filename, # regex
    archivo_nombre_original = old_filename, # regex
    es_audible
    )
...
# unimos con camera_sitio y equip_camera
soundscape_equipo <- soundscape_sitio %>%
  select(
    equipment_id = id.y, 
    microphone_id = id.x
  )
...
Imagen_referencia_microfonos <- archivos_microfono_ref %>%
  left_join(soundscape_equipo, by = "equipment_id") %>% 
  select(id, 
    grabadora_id = microphone_id, 
    archivo = filename, 
    archivo_nombre_original = old_filename)
