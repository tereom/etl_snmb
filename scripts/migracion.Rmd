...
dbConnect(RPostgreSQL::PostgreSQL(), "fpardo@localhost/snmb_v1", "Kamatar1")
...
cat_transecto_he_ids$id_sec <- 1:length(cat_transecto_hs_ids$id)

cat_camara_ids <- data.frame(
  id = unique(Camara$id))
cat_camara_ids$id_sec <- 1:length(cat_camara_ids$id)

cat_grabadora_ids <- data.frame(
  id = unique(Grabadora$id))
cat_grabadora_ids$id_sec <- 1: length(cat_grabadora_ids$id)

Camara <- Camara %>%
  inner_join(cat_camara_ids, by ="id") %>%
  mutate
```
